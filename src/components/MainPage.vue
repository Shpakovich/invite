    ...
    export default {
        ...
        computed: {
            appeal(){
                ...
                if(this.guests?.g){
                    this.guests.g === 'm' ? appeal = 'Дорогой ' : appeal = 'Дорогая ';
                } else {
                    appeal = 'Дорогие ';
                }
            ...
            appealPerson(){
                let appealPerson = 'вас';
                if (!this.guests?.name2 && !this.guests?.surname1) {
                    appealPerson = 'тебя';
                }
                return appealPerson;
            }
...
<template>
  ...
  <v-container class="pt-24 xl:pt-48">
      ...
      <section class="flex flex-column justify-center align-middle appeal-block">
          <h1 class="mb-6 mx-4 text-2xl sm:text-4xl xl:text-5xl" style="color:#6B483C; background-color: transparent!important;">{{ appeal }} мы, Диана и Денис, с удовольствием приглашаем {{ appealPerson }} провести с нами наш особенный день.<br>Будем очень рады {{appealPerson}} видеть 1 сентября на нашей свадьбе. 🎉</h1>
      ...
      <section style="justify-content: end" class="flex flex-column videoWrapper align-middle">
          ...
          <div class="text-opacity grid grid-flow-row md:grid-flow-col md:grid-cols-1 crimea-second-block">
              <p class="text-justify text-white hidden md:block font-weight-light crimea-text z-30">
                  ...
                  <br>Мы очень ждём {{appealPerson}} и надеемся, что радостно и комфортно будет всем!
              </p>
          </div>
      </section>
